...
| `GEMINI_API_KEY` | Google AI Studioなどで発行したAPIキー。設定済みなら文章化に使用します。 |
| `GEMINI_MODEL` | 任意。未設定なら `gemini-2.5-flash` |

Apps Scriptの左メニューから「プロジェクトの設定」を開き、「スクリプト プロパティ」に上記キーを追加します。`WEATHERAPI_KEY` が未設定の場合はOpen-Meteoを使い、AIキー未設定でもルールベースの投稿は動きます。
