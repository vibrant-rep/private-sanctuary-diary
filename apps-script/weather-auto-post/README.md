...
| 設定キー | 内容 |
|---|---|
| `WEATHERAPI_KEY` | WeatherAPI.comで発行したAPIキー。設定済みならWeatherAPI.comを優先して使います。 |
| `GEMINI_API_KEY` | Google AI Studioなどで発行したAPIキー。設定済みなら今日の一言に使用します。 |
| `GEMINI_MODEL` | 任意。未設定なら `gemini-3.1-flash-lite` |

Apps Scriptの左メニューから「プロジェクトの設定」を開き、「スクリプト プロパティ」に上記キーを追加します。`WEATHERAPI_KEY` が未設定の場合はOpen-Meteoを使い、AIキー未設定でも表形式の投稿は動きます。
...
表示は気温だけにし、服装は時間帯ごとの体感温度をもとに短く判定します。天気列には降水確率、メモ欄には湿度と風の注意だけを簡潔に出します。

| 条件 | 補正 |
|---|---|
| 体感温度が高い | Tシャツ |
| 体感温度が低い | ジャケットまたはダウン |
| 雨の可能性がある | 天気列に降水確率を表示 |
| 雨の可能性が低い | 雨なし表現は表示しない |
| Gemini利用時 | 今日の天気データから自然な一文を追加 |
...
| API制限 | WeatherAPI.comとOpen-Meteoの両方が失敗した場合は、1時間後に `retryDailyWeatherOutfit` が最大3回まで自動再試行します。 |
| 一時トリガー | API失敗後だけトリガー一覧に `retryDailyWeatherOutfit` が一時的に表示され、成功後に自動削除されます。 |
| 帰属 | 投稿末尾に実際に使った天気APIの帰属を付けています。 |
